...
public static class CacheCleanDialog
{
        ...
        Action onCancel)
    {
        ...
        var infoLabel =
            Label.New(
                T("Cache directory: {0}\nTotal cached files: {1} ({2})", cacheDir, entries.Count, SizeHelper.FormatSize(totalCacheSize)));
        infoLabel.Xalign = 0;
        infoLabel.Wrap = true;
        ...
        Label summaryLabel)
    {
        ...
        summaryLabel.SetText(T("{0} files, {1} would be freed", candidates.Count, SizeHelper.FormatSize(totalSize)));
    }
